...
| 表情导出 | 右键聊天中的表情并选择「导出表情…」；目前仅适配 4.1.13 build 269631 arm64 核心库 |
...
表情导出会从当前消息的只读 XML 获取 MD5；消息未携带下载地址时，会通过微信
已经打开的 `emoticon.db` 连接只读缓存对应的腾讯 CDN/AES 元数据，不提取或保存
数据库密钥。旧 HTTP 地址会转为 HTTPS，并尝试适配的腾讯 CDN 备用节点。
下载后自动识别 GIF、WebP、PNG 或 JPEG，并用真实格式显示保存面板；
4.x 加密资源只在内存中解密。整个过程不会修改消息、表情库或聊天数据库。
...
- `STICKER_EXPORT_QT_READY` → 当前核心库的 Qt 表情菜单适配器已启用
- `STICKER_METADATA_HOOK_READY` → 已注册表情库只读元数据入口
- `STICKER_METADATA_READY rows=N` → 已在微信数据库线程内缓存 N 条下载元数据（日志不含 MD5、地址或密钥）
- `STICKER_MENU_READY backend=qt rendered=yes` → 本次右键菜单已创建表情导出控件
- `STICKER_FETCH_SUCCEEDED format=... bytes=...` → 已取得图片数据，尚未表示用户已保存
- `STICKER_EXPORT_SAVED format=...` → 图片已按检测到的真实格式保存（日志不含地址或表情标识）
...
本地回归测试：`bash tests/run.sh`。测试只操作临时文件，覆盖特征唯一性、限长内存读取、撤回判定、跳板返回后的寄存器保持、撤回/表情 XML、CDN 白名单与 HTTPS 转换、图片格式与 AES 解码、表情元数据刷新及 URL/密钥配对、64 位 ID、Qt 几何/行复用、标记裁剪和双架构注入/卸载往返，不代替实际微信消息测试。
...
2. 当前实测版本为 4.1.13 build 269631（arm64 核心库）；气泡标记和表情导出在其他 build 或 Intel 上会安全关闭
3. 仅用于技术研究，请自行承担使用风险
